...
module TurboShowcase
  class ReplacingMethodsController < ApplicationController
    def index
      main_folder_for_views = "#{Rails.root}/app/views/turbo_showcase/replacing_methods/"
      component_views = "#{main_folder_for_views}/components"

      @index_html = File.open("#{main_folder_for_views}/index.html.erb").read
      @div_with_id_html = File.open("#{component_views}/div_with_id.html.erb").read
      @happy_mood_file_html = File.open("#{component_views}/_happy_mood.html.erb").read
      @sad_mood_file_html = File.open("#{component_views}/_sad_mood.html.erb").read
