...
module TurboShowcase
  class UpdatingMethodsController < ApplicationController
    def index
      main_folder_for_views = "#{Rails.root}/app/views/turbo_showcase/updating_methods/components_for_update"
      ...
      @controller_file_ruby =
        File.open("#{Rails.root}/app/controllers/turbo_showcase/updating_methods_controller.rb").read
    end

    def sad_mood
      render turbo_stream: turbo_stream.replace("some-id-for-replace",
                                               partial: "turbo_showcase/replacing_methods/components_for_update/happy_mood")
    end

    def happy_mood
      render turbo_stream: turbo_stream.replace("some-id-for-replace",
                                               partial: "turbo_showcase/replacing_methods/components_for_update/sad_mood")
    end
  end
end
